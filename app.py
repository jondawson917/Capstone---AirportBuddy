...
def get_trip_by_passenger(id):
    """Retrieves trips for a given passenger"""
    trips = Trip.query.get_or_404(id)
...
def add_baggage(id):
    ...
    if form.validate_on_submit():
        ...
        new_baggage = Baggage(brand_photo=brand_photo, bag_type=bag_type, weight=weight, passenger_id=id, trip=passenger.trip)

        db.session.add(new_baggage)
        passenger.num_bags+=1
...
def get_bag_by_id(id):
    ...
    return render_template('display_baggage.html', baggage=baggage, passenger=passenger)
